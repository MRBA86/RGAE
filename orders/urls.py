...
urlpatterns = [
    ...
    path('select_payment/<int:order_id>/', SelectPaymentMethodView.as_view(), name='select_payment'),
    path('<int:order_id>/payment/', PaymentView.as_view(), name='payment'),
    path('<int:order_id>/payment_success', PaymentSuccessView.as_view(), name='payment_success'),
